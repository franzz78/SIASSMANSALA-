...
try {
    ...
    console.error("Supabase engine error:", err);
}
...
// LOGKAI LOGIN AMAN & RESPONSIF 100%
function loginSistem() {
    ...
    if (!u || !p) {
        Swal.fire('Peringatan', 'Username/Password tidak boleh kosong!', 'warning');
        return;
    }
    ...
    if (u === 'AdminSMANSALA#' && p === 'SIAS2026-27##') {
        ...
        .then(() => {
            ...
            const userMenu = document.getElementById('menu-pengguna');
            if (userMenu) userMenu.value = loggedInUser;
        });
    } else {
        Swal.fire('Gagal', 'Akun tidak cocok atau belum terdaftar!', 'error');
    }
}
...
async function muatDataPerKelas(kelasPilihan) {
    ...
    if (!tbody) return;
    if (!kelasPilihan) {
        tbody.innerHTML = '<tr><td colspan="3" style="text-align:center; color:#64748b;">Silakan pilih kelas terlebih dahulu.</td></tr>';
        return;
    }
    if (!supabase) return;
...
async function simpanDataSistem() {
    ...
    const { error } = await supabase.from('data_siswa').insert([{ kelas, nama_siswa: nama, mata_pelajaran: mapel, nilai_siswa: parseFloat(nilai), pengguna_petugas: pengguna }]);
    Swal.close();
...
function clearMapelCache() {
    ...
    muatPilihanMapel();
    muatDaftarMapelTab();
    Swal.fire('Reset', 'Daftar mata pelajaran kembali ke default.', 'success');
}
